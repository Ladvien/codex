...
mod insight_tools_tests {
    ...
    fn test_insight_tools_are_available() {
        ...
        for expected_tool in &expected_insight_tools {
            let tool = tools_array
                .iter()
                .find(|t| t["name"] == *expected_tool)
                .unwrap();
            let description = tool["description"].as_str().unwrap();
            assert!(
                description.starts_with("★"),
                "Tool '{}' description should start with ★ to indicate insight functionality",
                expected_tool
            );
        }
    }
    ...
    fn test_generate_insights_tool_schema() {
        ...
        let generate_insights = tools_array
            .iter()
        ...
        let insight_type_enum = schema["properties"]["insight_type"]["enum"]
            .as_array()
            .unwrap();
    ...
    fn test_search_insights_tool_schema() {
        ...
        let search_insights = tools_array
            .iter()
...
mod insight_tools_disabled_tests {
    ...
    fn test_insight_tools_not_available_without_feature() {
        ...
        for tool_name in &insight_tools {
            ...
            assert!(
                !found,
                "Insight tool '{}' should NOT be available when codex-dreams feature is disabled",
                tool_name
            );
        }
        ...
        for tool_name in &regular_tools {
            ...
            assert!(
                found,
                "Regular tool '{}' should always be available",
                tool_name
            );
        }
    }
    ...
    fn test_insight_tool_validation_fails_without_feature() {
        ...
        for tool_name in &insight_tools {
            ...
            assert!(
                result.is_err(),
                "Insight tool '{}' should be unknown when feature disabled",
                tool_name
            );
            assert!(
                result.unwrap_err().contains("Unknown tool"),
                "Should get 'Unknown tool' error for '{}' when feature disabled",
                tool_name
            );
        }
    }
...
mod general_tool_compatibility_tests {
    ...
    fn test_tool_list_maintains_backward_compatibility() {
        ...
        for tool_name in &original_tools {
            ...
            assert!(
                found,
                "Original tool '{}' must remain available for backward compatibility",
                tool_name
            );
        }
    }
...
mod placeholder_response_tests {
    ...
    fn test_insight_export_includes_sample_structure() {
        ...
        assert!(
            true,
            "Sample export structures should be provided in placeholders"
        );
    }
}
