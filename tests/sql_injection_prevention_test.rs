...
mod sql_injection_tests {
    ...
    fn test_database_identifier_validation() -> Result<()> {
        ...
        assert!(
            DatabaseSetup::validate_database_identifier("db\"; CREATE DATABASE evil; --").is_err()
        );
        assert!(
            DatabaseSetup::validate_database_identifier("db' UNION SELECT * FROM passwords")
                .is_err()
        );
        ...
        assert!(
            DatabaseSetup::validate_database_identifier("db\\..\\..\\windows\\system32").is_err()
        );
    ...
    fn test_vector_formatting_safety() -> Result<()> {
        ...
        assert!(
            safe_pattern.is_match(&formatted),
            "Formatted vector '{}' contains unsafe characters",
            formatted
        );
    ...
    fn test_unicode_and_encoding_attacks() -> Result<()> {
        ...
        for attack in unicode_attacks {
            ...
            assert!(
                ...
                "Unicode attack should be rejected: '{}'",
                attack
            );
        }
        Ok(())
