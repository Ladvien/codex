...
fn main() {
    ...
    println!(
        "  ✓ Average time per calculation: {:.3} ms",
        avg_time_per_calc
    );
    ...
    for batch_size in batch_sizes {
        ...
        if let Ok(result) = engine.batch_calculate_recall_probability(&batch_params) {
            ...
            println!(
                "  Batch size {}: {:.1} ms total, {:.3} ms/memory, {:.0} memories/sec",
                batch_size,
                elapsed.as_millis(),
    ...
    println!(
        "  Never accessed memory: {:.3} ms",
        elapsed.as_micros() as f64 / 1000.0
    );
    if let Ok(calc_result) = result {
        println!(
            "    Recall probability: {:.3}",
            calc_result.recall_probability
        );
    }
