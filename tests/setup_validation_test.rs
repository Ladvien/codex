...
async fn test_complete_setup_validation() -> Result<()> {
    ...
    assert!(
        ...
        "Database should be healthy after setup"
    );
    ...
    for (i, text) in test_texts.iter().enumerate() {
        info!(
            "Testing embedding generation for text {}/{}",
            i + 1,
            test_texts.len()
        );

        let embedding = timeout(Duration::from_secs(30), embedder.generate_embedding(text))
            .await
    ...
    assert!(
        embedding_health.response_time_ms > 0,
        "Response time should be measured"
    );
    assert!(
        embedding_health.embedding_dimensions > 0,
        "Embedding dimensions should be detected"
    );
    ...
    let auto_embedding = auto_embedder
        .generate_embedding("Auto-configuration test")
        .await?;
    assert!(
        !auto_embedding.is_empty(),
        "Auto-configured embedder should generate embeddings"
    );
    ...
    match fallback_embedder
        .generate_embedding_with_fallback("Fallback test")
        .await
    {
        Ok(embedding) => {
    ...
    assert!(
        avg_time_per_embedding < 5000,
        "Embedding generation should be reasonably fast (< 5s)"
    );
...
async fn test_configuration_validation() -> Result<()> {
    ...
    assert!(
        !default_config.database_url.is_empty(),
        "Database URL should not be empty"
    );
    assert!(
        !default_config.embedding.provider.is_empty(),
        "Embedding provider should not be empty"
    );
    assert!(
        !default_config.embedding.model.is_empty(),
        "Embedding model should not be empty"
    );

    // Test that limits are reasonable
    assert!(
        default_config.tier_config.working_tier_limit > 0,
        "Working tier limit should be positive"
    );
    assert!(
        default_config.tier_config.warm_tier_limit > 0,
        "Warm tier limit should be positive"
    );
    assert!(
        default_config.operational.max_db_connections > 0,
        "DB connections should be positive"
    );
