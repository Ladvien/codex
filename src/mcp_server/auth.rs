...
impl Default for MCPAuthConfig {
    fn default() -> Self {
        Self {
            enabled: false,
            jwt_secret: env::var("MCP_JWT_SECRET")
                .unwrap_or_else(|_| "change-me-in-production-super-secret-key-minimum-32-chars".to_string()),
    ...
    fn load_api_keys_from_env() -> HashMap<String, ApiKeyInfo> {
            ...
            match serde_json::from_str::<HashMap<String, Value>>(&keys_json) {
                Ok(keys) => {
                    for (key, info) in keys {
                        if let Ok(client_id) = info.get("client_id")
                            .and_then(|v| v.as_str())
                            .ok_or("Missing client_id") {
                            
                            let scopes = info.get("scopes")
                            ...
                            let expires_at = info.get("expires_at")
                                .and_then(|v| v.as_str())
        ...
        if api_keys.is_empty() {
            if let Ok(api_key) = env::var("MCP_API_KEY") {
                let client_id = env::var("MCP_CLIENT_ID")
                    .unwrap_or_else(|_| "default-client".to_string());
                
                api_keys.insert(api_key, ApiKeyInfo {
                    client_id,
...
impl MCPAuth {
    ...
    pub async fn authenticate_request(
        ...
        let auth_result = if let Some(auth_header) = headers.get("authorization") {
            ...
            self.validate_certificate(cert_thumbprint, &request_id).await
        ...
        if elapsed.as_millis() > self.config.performance_target_ms as u128 {
            warn!("Authentication took {}ms, exceeding target of {}ms", 
                  elapsed.as_millis(), self.config.performance_target_ms);
        }
        
        match auth_result {
            Ok(context) => {
                debug!("Authentication successful for client: {}", context.client_id);
    ...
    async fn validate_certificate(&self, thumbprint: &str, request_id: &str) -> Result<AuthContext> {
    ...
    fn has_required_scopes(&self, provided_scopes: &[String]) -> bool {
        ...
        self.config.require_scope.iter()
    ...
    pub fn validate_tool_access(&self, context: &AuthContext, tool_name: &str) -> Result<()> {
        ...
        let required_scope = match tool_name {
            "store_memory" | "harvest_conversation" | "migrate_memory" | "delete_memory" => "mcp:write",
            "search_memory" | "get_statistics" | "what_did_you_remember" | "get_harvester_metrics" => "mcp:read",
...
mod tests {
    ...
    fn create_test_config() -> MCPAuthConfig {
        let mut api_keys = HashMap::new();
        api_keys.insert("test-key-123".to_string(), ApiKeyInfo {
    ...
    async fn test_api_key_authentication() {
        ...
        headers.insert("authorization".to_string(), "ApiKey test-key-123".to_string());
    ...
    async fn test_certificate_authentication() {
        ...
        let mut headers = HashMap::new();
        headers.insert("x-client-cert-thumbprint".to_string(), "abc123def456".to_string());
    ...
    async fn test_tool_access_validation() {
        ...
        assert!(auth.validate_tool_access(&context, "get_statistics").is_ok());
        
        // Should deny write operations
        assert!(auth.validate_tool_access(&context, "store_memory").is_err());
        assert!(auth.validate_tool_access(&context, "delete_memory").is_err());
    }
