...
use crate::insights::{HealthStatus, ProcessingReport};
#[cfg(feature = "codex-dreams")]
...
impl Default for SchedulerConfig {
    fn default() -> Self {
        Self {
            ...
            max_processing_duration_minutes: 60, // 1 hour for large model processing
...
impl InsightScheduler {
    ...
    pub async fn new(
        ...
        let scheduler = JobScheduler::new()
            .await
            .map_err(|e| anyhow::anyhow!("Failed to initialize job scheduler: {}", e))?;
    ...
    async fn execute_processing_run(
        ...
        match processing_result {
            Ok(report) => {
                ...
                stats.avg_processing_duration_seconds = ((stats.avg_processing_duration_seconds
                    * (stats.successful_runs - 1) as f64)
                    + duration_seconds)
                    / stats.successful_runs as f64;
                stats.status = SchedulerStatus::Running;
            }
            Err(e) => {
    ...
    pub async fn shutdown(&mut self) -> Result<(), anyhow::Error> {
        ...
        // Stop the job scheduler
        {
            ...
            sched
                .shutdown()
                .await
                .map_err(|e| anyhow::anyhow!("Failed to shutdown job scheduler: {}", e))?;
        }
    ...
    pub async fn get_health_status(&self) -> HealthStatus {
        ...
        components.insert(
            "cron_engine".to_string(),
            matches!(
                stats.status,
                SchedulerStatus::Running | SchedulerStatus::Processing
            ),
        );
        // Calculate next scheduled run (placeholder until cron scheduler exposes this)
        let next_scheduled = if matches!(
            stats.status,
            SchedulerStatus::Running | SchedulerStatus::Processing
        ) {
