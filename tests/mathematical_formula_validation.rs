...
mod ebbinghaus_validation {
    ...
    fn test_ebbinghaus_curve_at_t_zero() {
        ...
        assert!(
            (result.recall_probability - 1.0).abs() < 0.001,
            "At t=0, retention should be 1.0, got {}",
    ...
    fn test_ebbinghaus_curve_at_strength_parameter() {
        ...
        assert!(
            (result.recall_probability - expected).abs() < 0.01,
            "At t=S, retention should be 1/e ≈ 0.368, got {}",
            result.recall_probability
        );
    }
    ...
    fn test_ebbinghaus_curve_research_benchmarks() {
        ...
        for (hours, expected) in test_cases {
            ...
            if expected > 0.001 {
                ...
                assert!(
                    (result.recall_probability - expected).abs() < tolerance,
                    "At {}h: expected {:.6}, got {:.6}",
                    hours,
                    expected,
                    result.recall_probability
                );
                ...
                assert!(
                    result.recall_probability < 0.001,
                    "At {}h: expected near zero, got {:.10}",
                    hours,
                    result.recall_probability
                );
            }
...
mod three_component_validation {
    ...
    fn test_recency_score_exponential_decay() {
        ...
        for hours in test_hours {
            ...
            assert!(
                (result.recency_score - expected).abs() < tolerance,
                "At {}h: expected {:.6}, got {:.6}",
                hours,
                expected,
    ...
    fn test_combined_score_weighted_average() {
        ...
        assert!(
            (result.combined_score - expected).abs() < 0.001,
        ...
        if let Some(explanation) = result.score_explanation {
            ...
            assert!(
                (explanation.relevance_contribution - 0.2 * result.relevance_score).abs() < 0.001
            );
        } else {
    ...
    fn test_score_bounds_validation() {
        ...
        assert!(
            result.recency_score >= 0.0 && result.recency_score <= 1.0,
            "Recency score out of bounds: {}",
            result.recency_score
        );
        assert!(
            result.importance_score >= 0.0 && result.importance_score <= 1.0,
            "Importance score out of bounds: {}",
...
mod performance_validation {
    ...
    fn test_calculation_performance_requirements() {
        ...
        assert!(
            result.calculation_time_ms <= 10,
            "Calculation exceeded 10ms target: {}ms",
    ...
    fn test_three_component_performance_target() {
        ...
        assert!(
            result.calculation_time_ms <= 5,
            "Three-component scoring exceeded 5ms target: {}ms",
            result.calculation_time_ms
        );
    }
    ...
    fn test_batch_processing_throughput() {
        ...
        assert!(
            throughput >= 100.0, // More lenient for smaller batches
            "Batch throughput too low: {:.1} memories/second",
            throughput
        );
...
mod accuracy_validation {
    ...
    fn test_mathematical_tolerance_requirement() {
        ...
        assert!(engine
            .validate_accuracy(0.5, 0.5 + tolerance + 0.0001)
            .is_err());
    }
    ...
    fn test_consolidation_strength_bounds() {
        ...
        assert!(
            ...
            result_max.recall_probability,
            result_min.recall_probability
        );
    }
    ...
    fn test_edge_case_handling() {
        ...
        assert!(
            (result_zero.recall_probability - 1.0).abs() < 0.001,
        ...
        assert!(
            result_never.recall_probability >= 0.0 && result_never.recall_probability <= 1.0,
...
mod integration_validation {
    ...
    fn test_cross_module_consistency() {
        ...
        let scoring_result = scoring_engine
            .calculate_score(&memory, &context, false)
            .unwrap();
