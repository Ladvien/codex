...
async fn test_insights_migration_syntax() -> Result<()> {
    ...
    let rollback_content = fs::read_to_string(ROLLBACK_FILE)
        .context("Failed to read rollback file")?;
    
    // Basic syntax validation
    assert!(!migration_content.is_empty(), "Migration file should not be empty");
    assert!(!rollback_content.is_empty(), "Rollback file should not be empty");
    
    // Check for required keywords in migration
    assert!(migration_content.contains("CREATE TABLE IF NOT EXISTS insights"), "Should create insights table");
    assert!(migration_content.contains("CREATE TABLE IF NOT EXISTS insight_vectors"), "Should create insight_vectors table");
    assert!(migration_content.contains("CREATE TABLE IF NOT EXISTS insight_feedback"), "Should create insight_feedback table");
    assert!(migration_content.contains("CREATE TABLE IF NOT EXISTS processing_queue"), "Should create processing_queue table");
    assert!(migration_content.contains("ALTER TABLE memories ADD COLUMN IF NOT EXISTS processing_metadata"), "Should add processing_metadata column");
    
    // Check for required keywords in rollback
    assert!(rollback_content.contains("DROP TABLE IF EXISTS insights"), "Should drop insights table");
    assert!(rollback_content.contains("DROP TABLE IF EXISTS insight_vectors"), "Should drop insight_vectors table");
    assert!(rollback_content.contains("DROP TABLE IF EXISTS insight_feedback"), "Should drop insight_feedback table");
    assert!(rollback_content.contains("DROP TABLE IF EXISTS processing_queue"), "Should drop processing_queue table");
    assert!(rollback_content.contains("ALTER TABLE memories DROP COLUMN IF EXISTS processing_metadata"), "Should drop processing_metadata column");
    
    // Check for feature flag handling
    assert!(migration_content.contains("codex-dreams"), "Should check codex-dreams feature flag");
    assert!(migration_content.contains("CODEX_DREAMS_ENABLED"), "Should check environment variable");
    
    // Check for performance optimizations
    assert!(migration_content.contains("HNSW"), "Should create HNSW vector index");
    assert!(migration_content.contains("m = 48"), "Should use optimized HNSW parameters");
    assert!(migration_content.contains("ef_construction = 200"), "Should use optimized ef_construction");
...
async fn test_migration_with_database() -> Result<()> {
    ...
    assert_eq!(column_exists, 1, "processing_metadata column should be added to memories");
    ...
    sqlx::query(
        "INSERT INTO memories (id, content, content_hash, tier) VALUES ($1, $2, $3, $4)"
    )
    .bind(memory_id)
    ...
    let insight_count: i64 = sqlx::query_scalar(
        "SELECT COUNT(*) FROM insights WHERE id = $1"
    )
    .bind(insight_id)
    .fetch_one(&pool)
    .await?;
    ...
    assert_eq!(remaining_tables, 0, "All insights tables should be removed after rollback");
    ...
    assert_eq!(column_remains, 0, "processing_metadata column should be removed after rollback");
...
async fn test_migration_performance() -> Result<()> {
    ...
    assert!(migration_duration.as_secs() < 30, 
        "Migration took too long: {}s", migration_duration.as_secs());
    ...
    sqlx::query(
        "INSERT INTO insight_vectors (insight_id, embedding) VALUES ($1, $2)"
    )
    .bind(Uuid::new_v4())
    ...
    assert!(vector_insert_duration.as_millis() < 1000,
        "Vector insert took too long: {}ms", vector_insert_duration.as_millis());
    
    println!("✅ Performance test passed - Migration: {}s, Vector insert: {}ms", 
        migration_duration.as_secs(), vector_insert_duration.as_millis());
...
fn test_schema_documentation_exists() {
    ...
    assert!(std::path::Path::new("docs/database/insights-schema.md").exists(),
    ...
    assert!(doc_content.contains("## Schema Tables"), "Should document schema tables");
    assert!(doc_content.contains("## Index Strategy"), "Should document indexing strategy");
    assert!(doc_content.contains("## Performance Targets"), "Should document performance targets");
    assert!(doc_content.contains("## Feature Flag Implementation"), "Should document feature flags");
