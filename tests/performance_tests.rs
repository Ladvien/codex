...
use std::sync::Arc;
use test_helpers::{ConcurrentTester, PerformanceMeter, TestConfigBuilder, TestDataGenerator, TestEnvironment};
...
async fn test_baseline_operation_performance() -> Result<()> {
    ...
    let search_results = env.test_search("baseline performance test", Some(10)).await?;
...
async fn test_high_volume_memory_creation() -> Result<()> {
    ...
    for batch_size in batch_sizes {
        ...
        println!("Batch size {}: {:.1} ops/sec, total time: {:?}", 
        ...
        match batch_size {
            10 => {
                ...
                assert!(ops_per_second >= 1.0, "Should achieve at least 1 op/sec for small batches");
            }
            50 => {
                batch_result.assert_under(Duration::from_secs(30));
                assert!(ops_per_second >= 1.5, "Should achieve at least 1.5 ops/sec for medium batches");
            }
            100 => {
                batch_result.assert_under(Duration::from_secs(60));
                assert!(ops_per_second >= 1.5, "Should maintain performance for larger batches");
            }
            250 => {
                batch_result.assert_under(Duration::from_secs(180)); // 3 minutes for large batch
                assert!(ops_per_second >= 1.0, "Should maintain minimum performance for very large batches");
            }
            _ => {}
        }
...
async fn test_concurrent_user_simulation() -> Result<()> {
    ...
    for user_count in user_counts {
        ...
        println!("Concurrent users {}: {:.1} ops/sec, total time: {:?}", 
        ...
        match user_count {
            5 => {
                ...
                assert!(ops_per_second >= 1.0, "Should maintain performance with 5 users");
            }
            10 => {
                concurrent_result.assert_under(Duration::from_secs(90));
                assert!(ops_per_second >= 0.8, "Should maintain reasonable performance with 10 users");
            }
            20 => {
                concurrent_result.assert_under(Duration::from_secs(180)); // 3 minutes
                assert!(ops_per_second >= 0.5, "Should maintain minimum performance with 20 users");
            }
            _ => {}
        }
...
async fn test_search_performance_scaling() -> Result<()> {
    ...
    println!("Creating dataset of {} memories for search testing", dataset_size);
    ...
    for (query, limit) in search_scenarios {
        let search_meter = PerformanceMeter::new(&format!("search_{}_{}", query.replace(" ", "_"), limit));
        ...
        let search_request = SearchRequest {
            ...
            date_range: None,
            importance_range: None,
            metadata_filters: Some(serde_json::json!({"test_id": env.test_id})),
            tags: None,
            limit: Some(limit),
            offset: None,
            cursor: None,
        ...
        println!("Search '{}' (limit {}): {:?}, found {} results", 
            query, limit, search_result.duration, search_response.results.len());
    ...
    let concurrent_searches = ConcurrentTester::run_parallel(
        |i| {
            let repo = Arc::clone(&env.repository);
            let test_id = env.test_id.clone();
            async move {
    ...
    println!("Concurrent searches: {:.1} searches/sec, total time: {:?}", 
        search_ops_per_sec, concurrent_search_result.duration);

    concurrent_search_result.assert_under(Duration::from_secs(10));
    assert!(search_ops_per_sec >= 1.5, "Should achieve good concurrent search performance");
...
async fn test_memory_tier_performance() -> Result<()> {
    ...
    println!("Creating {} memories per tier for tier performance testing", memories_per_tier);
    ...
    for tier in &tiers {
        ...
        let tier_operations = ConcurrentTester::run_parallel(
            |i| {
                ...
                async move {
                    let request = CreateMemoryRequest {
                        content: format!("Tier {:?} memory {} for performance testing with sufficient content", tier, i),
                        embedding: None,
                        tier: Some(tier),
        ...
        println!("Created {} {:?} tier memories in {:?}", 
    ...
    for tier in &tiers {
        ...
        println!("Found {} results in {:?} tier", tier_results.results.len(), tier);
    ...
    for memory in &all_memories[0..memories_per_tier] { // Working tier memories
        for _ in 0..5 {
    ...
    for memory in &all_memories[memories_per_tier..memories_per_tier*2] { // Warm tier memories
        for _ in 0..2 {
            let _ = env.repository.get_memory(memory.id).await?;
        }
    }
    ...
    for memory in &all_memories[memories_per_tier*2..] { // Cold tier memories
        let _ = env.repository.get_memory(memory.id).await?;
    }

    let access_pattern_result = access_pattern_meter.finish();
    println!("Tier access pattern simulation took: {:?}", access_pattern_result.duration);
...
async fn test_embedding_generation_performance() -> Result<()> {
    ...
    for (size_name, char_count) in content_sizes {
        println!("Testing embedding generation for {} content ({} chars)", size_name, char_count);
        
        let content = TestDataGenerator::large_content(char_count / 1024 + 1)[..char_count].to_string();
        ...
        println!("Embedding generation for {} content: {:?} (dimension: {})", 
            size_name, embedding_result.duration, embedding.len());
    ...
    let batch_content: Vec<String> = (0..20)
        .map(|i| format!("Batch embedding test content {} with sufficient length for realistic testing", i))
        .collect();

    let batch_embeddings = env.embedder.generate_embeddings_batch(&batch_content).await?;
    
    let batch_result = batch_meter.finish();
    println!("Batch embedding generation (20 items): {:?}", batch_result.duration);
...
async fn test_resource_usage_and_limits() -> Result<()> {
    ...
    for batch_size in batch_sizes {
        ...
        println!("Batch size {}: {:.1} ops/sec, duration: {:?}", 
        ...
        println!("Search with {} memories: {:?}, found {} results", 
            batch_size, search_duration, search_results.len());

        // Performance should not degrade significantly with larger datasets
        assert!(search_duration < Duration::from_secs(2), 
            "Search performance should remain good even with {} memories", batch_size);
    }

    let memory_usage_result = memory_usage_meter.finish();
    println!("Total resource usage test duration: {:?}", memory_usage_result.duration);
    ...
    println!("Connection efficiency test (50 operations): {:?}", connection_result.duration);
