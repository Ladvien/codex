...
use codex_memory::memory::{MemoryRepository, CreateMemoryRequest, MemoryTier};
...
async fn test_e2e_search_memory_mcp_tool_fulltext() {
    ...
    let repository = Arc::new(MemoryRepository::new(pool.clone(), embedding_provider.clone()));
    ...
    assert!(fulltext_result.is_ok(), "Fulltext search through MCP tool should not fail: {:?}", fulltext_result.err());
    
    let fulltext_response = fulltext_result.unwrap();
    assert!(fulltext_response.is_object(), "Response should be JSON object");
    
    let results = fulltext_response.get("results").expect("Response should have results");
    ...
    assert!(memory.get("content").is_some(), "Memory should have content");
    assert!(memory.get("importance_score").is_some(), "Memory should have importance_score");
    assert!(memory.get("recency_score").is_some(), "Memory should have recency_score");
    assert!(memory.get("relevance_score").is_some(), "Memory should have relevance_score");
    ...
    assert!(hybrid_result.is_ok(), "Hybrid search through MCP tool should not fail: {:?}", hybrid_result.err());
    
    let hybrid_response = hybrid_result.unwrap();
    let hybrid_results = hybrid_response.get("results").unwrap().as_array().unwrap();
    assert!(!hybrid_results.is_empty(), "Hybrid search should find test memory");
    
    // Verify score explanation is included when requested
    let first_hybrid_result = &hybrid_results[0];
    assert!(first_hybrid_result.get("score_explanation").is_some(), "Should have score explanation when requested");
    ...
    match edge_result {
        ...
        Err(e) => {
            ...
            assert!(!error_msg.contains("column"), "Should not have column-related errors: {}", error_msg);
            assert!(!error_msg.contains("try_get"), "Should not have column mapping errors: {}", error_msg);
            println!("✅ Edge case properly rejected with non-column error: {}", error_msg);
        }
    }

    // Cleanup test memory
    repository.delete_memory(test_memory.id, Some("E2E test cleanup".to_string())).await
...
async fn test_e2e_ohat_memory_search_mcp() {
    ...
    assert!(ohat_result.is_ok(), "OHAT search should not fail: {:?}", ohat_result.err());
    ...
    if has_ohat {
        ...
        if found_target {
            println!("✅ Found the specific OHAT memory that was previously failing: {}", target_id);
        } else {
            println!("ℹ️  OHAT memories found, but not the specific target ID (may have been deleted)");
        }
...
async fn test_e2e_search_consistency_prevention() {
    ...
    let comprehensive_memory_request = CreateMemoryRequest {
        content: "CONSISTENCY_TEST comprehensive memory with all fields for structure validation".to_string(),
        metadata: Some(json!({
    ...
    let test_memory = repository.create_memory(comprehensive_memory_request).await
    ...
    for search_type in search_types {
        ...
        assert!(result.is_ok(), "{} search should not fail: {:?}", search_type, result.err());
        
        let response = result.unwrap();
        let results = response.get("results").unwrap().as_array().unwrap();
        assert!(!results.is_empty(), "{} search should find test memory", search_type);
        
        let first_result = &results[0];
        let memory = first_result.get("memory").unwrap();
        
        // Extract all field names for consistency checking
        let field_names: Vec<String> = memory.as_object().unwrap()
            .keys()
            .cloned()
            .collect();
        ...
        let critical_fields = [
            "id", "content", "tier", "status", "importance_score", 
            "recency_score", "relevance_score", "access_count",
            "created_at", "updated_at", "metadata"
        ];
        
        for field in critical_fields.iter() {
            assert!(memory.get(field).is_some(), 
                "{} search missing critical field '{}': available fields: {:?}", 
                search_type, field, memory.as_object().unwrap().keys().collect::<Vec<_>>());
        }
        
        // Verify result-level fields
        assert!(first_result.get("similarity_score").is_some(), 
            "{} search should have similarity_score", search_type);
    ...
    for field in fulltext_fields {
        assert!(hybrid_fields.contains(field), 
            "Field '{}' present in {} but missing in {}", field, fulltext_type, hybrid_type);
    }
    
    for field in hybrid_fields {
        assert!(fulltext_fields.contains(field), 
            "Field '{}' present in {} but missing in {}", field, hybrid_type, fulltext_type);
    }

    println!("✅ Search structure consistency verified between {} and {}", fulltext_type, hybrid_type);

    // Cleanup
    repository.delete_memory(test_memory.id, Some("E2E consistency test cleanup".to_string())).await
        .expect("Failed to cleanup test memory");
