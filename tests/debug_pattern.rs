#[cfg(test)]
mod tests {
    use codex_memory::memory::event_triggers::{TriggerPattern, TriggerEvent, TriggerConfig};
    ...
    fn debug_pattern_matching() {
        ...
        for (trigger_type, pattern) in &config.patterns {
            if pattern.matches(content) {
                ...
                println!("{:?} pattern: confidence = {:.3}, threshold = {:.3}", 
                         trigger_type, confidence, pattern.confidence_threshold);
                println!("  Keywords: {:?}", pattern.keywords);
                
                let content_lower = content.to_lowercase();
                let matching_keywords: Vec<&String> = pattern.keywords.iter()
