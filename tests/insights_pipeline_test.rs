...
use codex_memory::{
    ...
    memory::{CreateMemoryRequest, MemoryRepository, MemoryTier, SearchRequest, SearchType},
};
use serde_json::json;
use sqlx::PgPool;
...
async fn test_memory_retrieval(repository: &MemoryRepository) -> Result<()> {
    ...
    assert!(
        ...
        "Temporal search should return test memories"
    );

    // Verify the results contain our test data
    let found_test_memory = search_results.results.iter().any(|result| {
        result
            .memory
            .metadata
            .get("created_by")
    ...
    assert!(
        found_test_memory,
        "Should find our test memories in temporal search results"
    );
...
async fn test_insights_scheduler() -> Result<()> {
    ...
    info!(
        ...
        run_result.success, run_result.duration_seconds
    );

    assert!(run_result.success, "Manual scheduler run should succeed");
    assert!(
        run_result.duration_seconds.is_some(),
...
async fn test_insights_processor_integration() -> Result<()> {
    ...
    info!(
        "Scheduler run with no processor: success={}",
        run_result.success
    );
    ...
    assert!(
        run_result.success,
        "Scheduler should succeed even without processor"
    );
    ...
    if let Some(report) = &run_result.processing_report {
        ...
        assert_eq!(
            ...
            "Fallback should have 100% success rate"
        );
    }
...
async fn test_complete_insights_pipeline() -> Result<()> {
    ...
    cleanup_test_data(&pool).await.unwrap_or_else(|e| {
        warn!(
            "Initial cleanup failed (expected if no test data exists): {}",
            e
        );
    });
...
async fn test_temporal_search_fix_verification() -> Result<()> {
    ...
    let found_memory = search_results
        .results
        .iter()
