...
impl InsightExporter {
    ...
    pub async fn export_markdown(&self, filter: ExportFilter) -> Result<String> {
        ...
        info!(
            "Successfully generated Markdown export with {} insights",
            export.metadata.total_insights
        );
        Ok(markdown)
    }
    ...
    pub async fn export_jsonld(&self, filter: ExportFilter) -> Result<String> {
        ...
        info!(
            "Successfully generated JSON-LD export with {} insights",
            export.metadata.total_insights
        );
        Ok(jsonld)
    }
    ...
    fn create_export(
        ...
        Ok(InsightExport { insights, metadata })
    }
    ...
    fn check_size_limit(&self, content: &str, format: &str) -> Result<()> {
        ...
        debug!(
            "{} export size: {} bytes (within {} byte limit)",
            format, size, MAX_EXPORT_SIZE
        );
        Ok(())
    }
