...
mod security_migration_tests {
    ...
    async fn test_statement_timeout_configuration() -> Result<()> {
        ...
        assert_eq!(row.0, "30s", "Statement timeout should be set to 30 seconds");
    ...
    async fn test_idle_transaction_timeout_configuration() -> Result<()> {
        ...
        let row: (String,) = sqlx::query_as("SELECT current_setting('idle_in_transaction_session_timeout')")
            .fetch_one(&pool)
            .await?;
        
        // Should be 60s (60000ms) after migration
        assert_eq!(row.0, "60s", "Idle transaction timeout should be set to 60 seconds");
    ...
    async fn test_statement_timeout_enforcement() -> Result<()> {
        ...
        match long_query_result {
            Ok(query_result) => {
                ...
                assert!(query_result.is_err(), "Query should have been terminated by statement timeout");
                
                // Check if it's a timeout error
                let error = query_result.unwrap_err();
                let error_message = error.to_string();
                assert!(
                    error_message.contains("timeout") || error_message.contains("canceling statement"),
                    "Error should indicate timeout: {}", error_message
                );
            },
            Err(_) => {
    ...
    async fn test_security_monitoring_view() -> Result<()> {
        ...
        assert!(result.is_ok(), "Security monitoring view should be accessible");
        ...
        assert!(!rows.is_empty(), "Security monitoring should show active connections");
    ...
    async fn test_connection_pool_limits() -> Result<()> {
        ...
        let max_connections: i32 = row.0.parse()
            .context("Failed to parse max_connections value")?;
        
        // Should be reasonable limit (typically 200)
        assert!(max_connections >= 100, "Max connections should be at least 100");
        assert!(max_connections <= 300, "Max connections should not exceed 300");
    ...
    async fn test_query_logging_configuration() -> Result<()> {
        ...
        assert_eq!(row.0, "1s", "Query logging threshold should be set to 1 second");
    ...
    async fn test_autovacuum_configuration() -> Result<()> {
        ...
        let row: (String,) = sqlx::query_as("SELECT current_setting('autovacuum_vacuum_scale_factor')")
            .fetch_one(&pool)
            .await?;
        
        let scale_factor: f64 = row.0.parse()
            .context("Failed to parse autovacuum_vacuum_scale_factor")?;
        
        assert_eq!(scale_factor, 0.1, "Autovacuum vacuum scale factor should be 0.1");
    ...
    async fn test_migration_rollback() -> Result<()> {
        ...
        let original_statement_timeout: (String,) = sqlx::query_as("SELECT current_setting('statement_timeout')")
            .fetch_one(&pool)
            .await?;
        
        let original_idle_timeout: (String,) = sqlx::query_as("SELECT current_setting('idle_in_transaction_session_timeout')")
            .fetch_one(&pool)
            .await?;
        ...
        let new_statement_timeout: (String,) = sqlx::query_as("SELECT current_setting('statement_timeout')")
            .fetch_one(&mut *tx)
            .await?;
            
        let new_idle_timeout: (String,) = sqlx::query_as("SELECT current_setting('idle_in_transaction_session_timeout')")
            .fetch_one(&mut *tx)
            .await?;
        
        assert_eq!(new_statement_timeout.0, "300s", "Rollback should set statement timeout to 300s");
        assert_eq!(new_idle_timeout.0, "600s", "Rollback should set idle timeout to 600s");
        
        // Rollback transaction to restore original values
        tx.rollback().await?;
        
        // Verify original values are restored
        let restored_statement_timeout: (String,) = sqlx::query_as("SELECT current_setting('statement_timeout')")
            .fetch_one(&pool)
            .await?;
            
        assert_eq!(restored_statement_timeout.0, original_statement_timeout.0, 
