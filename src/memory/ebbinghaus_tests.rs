...
mod ebbinghaus_validation_tests {
    ...
    fn test_ebbinghaus_empirical_data_alignment() {
        ...
        for &(time_hours, expected_retention) in EBBINGHAUS_EMPIRICAL_DATA {
            let calculated_retention =
                calculate_ebbinghaus_retention(time_hours, empirical_strength);

            // Allow 5% tolerance for empirical data fitting
            assert_relative_eq!(calculated_retention, expected_retention, epsilon = 0.05);
        }
    }
    ...
    fn test_edge_cases() {
        ...
        assert!(
            retention > 0.999,
            "Very recent memories should have ~100% retention"
        );

        // Test zero strength (should not panic)
        let result = std::panic::catch_unwind(|| calculate_ebbinghaus_retention(1.0, 0.0));
        assert!(
            result.is_err(),
            "Zero strength should cause controlled failure"
        );
...
mod integration_tests {
    ...
    fn test_never_accessed_memory_ebbinghaus() {
        ...
        assert_relative_eq!(result.recall_probability, expected_retention, epsilon = 0.1);
    }
