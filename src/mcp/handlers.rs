use crate::memory::{
    models::{CreateMemoryRequest, SearchRequest, SearchResult, UpdateMemoryRequest},
    Memory, MemoryError, MemoryRepository, MemoryTier,
};
...
pub struct MemoryHandlerImpl {
    ...
    #[allow(dead_code)]
    circuit_breaker: Arc<crate::mcp::circuit_breaker::CircuitBreaker>,
    retry_policy: Arc<crate::mcp::retry::RetryPolicy>,
}
...
impl MemoryHandler for MemoryHandlerImpl {
    ...
    async fn create_memory(&self, request: CreateMemoryRequest) -> Result<Memory, MemoryError> {
        debug!(
            "Creating memory with content length: {}",
            request.content.len()
        );
        ...
        let result = self
            .retry_policy
            .execute(|| async { repo.create_memory(request.clone()).await })
            .await;
    ...
    async fn get_memory(&self, id: Uuid) -> Result<Memory, MemoryError> {
        ...
        let result = self
            .retry_policy
            .execute(|| async { repo.get_memory(id).await })
            .await;
    ...
    async fn update_memory(
        ...
        let result = self
            .retry_policy
            .execute(|| async { repo.update_memory(id, request.clone()).await })
            .await;
    ...
    async fn delete_memory(&self, id: Uuid) -> Result<(), MemoryError> {
        ...
        let result = self
            .retry_policy
            .execute(|| async { repo.delete_memory(id).await })
            .await;
    ...
    async fn search_memories(
        ...
        let result = self
            .retry_policy
            .execute(|| async { repo.search_memories(request.clone()).await })
            .await;

        match &result {
            Ok(results) => info!("Found {} memories matching search", results.results.len()),
            Err(e) => error!("Failed to search memories: {}", e),
        }

        result.map(|response| response.results)
    }
    ...
    async fn migrate_memory(
        ...
        let result = self
            .retry_policy
            .execute(|| async { repo.migrate_memory(id, to_tier, reason.clone()).await })
            .await;
