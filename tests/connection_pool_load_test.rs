...
mod connection_pool_load_tests {
    ...
    async fn test_concurrent_vector_searches() -> anyhow::Result<()> {
        let database_url = std::env::var("DATABASE_URL").unwrap_or_else(|_| {
            "postgres://postgres:postgres@localhost/codex_memory_test".to_string()
        });
        ...
        assert!(
            successes > 40,
            "Expected >40 successful searches, got {}",
            successes
        );
        assert!(
            duration < Duration::from_secs(20),
            "Test took too long: {:.2}s",
            duration.as_secs_f64()
        );
    ...
    async fn test_pool_recovery_from_exhaustion() -> anyhow::Result<()> {
        ...
        // Verify recovery
        let recovery_result = sqlx::query("SELECT 1").fetch_one(&*pool).await;

        assert!(
            recovery_result.is_ok(),
            "Pool should recover after connection release"
        );
...
mod integration_tests {
    ...
    async fn test_concurrent_vector_insertions() -> anyhow::Result<()> {
        ...
        assert!(
            successful_insertions >= insertion_count * 95 / 100,
