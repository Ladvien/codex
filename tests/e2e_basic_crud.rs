...
async fn test_database_basic_operations() -> Result<()> {
    ...
    let inserted_id: Uuid = sqlx::query_scalar(r#"
    ...
    let retrieved_row = sqlx::query(r#"
    ...
    let search_results = sqlx::query(r#"
        SELECT id, content, metadata
        FROM test_memories
    ...
    println!("✅ Successfully performed search operations (found {} results)", search_results.len());
...
async fn test_ollama_embedding_integration() -> Result<()> {
    ...
    assert_eq!(embedding.len(), 768, "Expected 768-dimensional embedding for nomic-embed-text");
    assert!(!embedding.is_empty(), "Embedding should not be empty");
    
    // Verify embedding is roughly normalized (cosine similarity compatible)
    let magnitude: f32 = embedding.iter().map(|x| x * x).sum::<f32>().sqrt();
    assert!(magnitude > 0.5, "Embedding magnitude should be reasonable: {}", magnitude);
    ...
    for (i, embedding) in batch_embeddings.iter().enumerate() {
        assert_eq!(embedding.len(), 768, "All embeddings should have same dimensions");
        let magnitude: f32 = embedding.iter().map(|x| x * x).sum::<f32>().sqrt();
        assert!(magnitude > 0.5, "Embedding {} magnitude should be reasonable: {}", i, magnitude);
    }
    
    // Verify embeddings are different for different content
    assert_ne!(batch_embeddings[0], batch_embeddings[1], "Different content should produce different embeddings");
    assert_ne!(batch_embeddings[1], batch_embeddings[2], "Different content should produce different embeddings");
    ...
    println!("  - Throughput: {:.1} embeddings/second", embeddings_per_second);
    ...
    assert!(similarity > 0.99, "Same content should produce very similar embeddings: {:.4}", similarity);
    ...
    let performance_tests = vec![
        ...
        ("Medium text", "This is a medium-length text for testing embedding generation performance."),
        ("Long text", &long_text),
    ];
    ...
    for (name, duration) in &performance_results {
        assert!(duration.as_secs() < 10, "{} took too long: {:?}", name, duration);
    }
...
async fn test_comprehensive_e2e_flow() -> Result<()> {
    ...
    let test_memories = vec![
        ("User prefers dark mode interface", json!({"type": "preference", "category": "ui"})),
        ("Database connection uses PostgreSQL at 192.168.1.104", json!({"type": "config", "category": "database"})),
        ("Embedding model is nomic-embed-text running on Ollama", json!({"type": "config", "category": "ai"})),
        ("System supports three memory tiers: working, warm, cold", json!({"type": "architecture", "category": "memory"})),
    ];
    ...
    for (content, metadata) in &test_memories {
        ...
        let memory_id: Uuid = sqlx::query_scalar(r#"
    ...
    for memory_id in &memory_ids {
        ...
        assert_eq!(embedding.len(), 768, "Stored embedding should maintain dimensions");
    ...
    assert!(similarities[0].1 > 0.5, "Most similar memory should have reasonable similarity score: {:.4}", similarities[0].1);
    assert!(similarities[0].0.contains("Database") || similarities[0].0.contains("PostgreSQL"), 
        "Most similar memory should be database-related: {}", similarities[0].0);
    ...
    let working_memories = sqlx::query(r#"
        SELECT COUNT(*) as count FROM e2e_test_memories WHERE tier = 'working'
    "#)
    .fetch_one(&pool)
    .await?;
    
    let warm_memories = sqlx::query(r#"
    ...
    assert!(performance_time.as_millis() < 1000, "Batch queries should complete quickly");
