...
async fn main() -> Result<()> {
    ...
    let retrieve_query = "SELECT content, insight_type, confidence_score FROM insights WHERE id = $1";
    let (content, insight_type, confidence): (String, String, f64) = 
        sqlx::query_as(retrieve_query)
            .bind(result.0)
            .fetch_one(pool.as_ref())
            .await?;
