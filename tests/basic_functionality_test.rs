...
async fn test_embedding_service_creation() -> Result<()> {
    ...
    assert_eq!(*ollama_embedder.provider(), codex_memory::embedding::EmbeddingProvider::Ollama);
    ...
    println!("  - Ollama embedder: {} dimensions", ollama_embedder.embedding_dimension());
    println!("  - Mock embedder: {} dimensions", mock_embedder.embedding_dimension());
    println!("  - OpenAI embedder: {} dimensions", openai_embedder.embedding_dimension());
...
async fn test_mock_embedding_generation() -> Result<()> {
    ...
    let embedding = embedder.generate_embedding("Test content for embedding").await?;
    assert_eq!(embedding.len(), 768);
    assert!(!embedding.is_empty());
    
    // Test that the same content produces the same embedding (deterministic)
    let embedding2 = embedder.generate_embedding("Test content for embedding").await?;
    assert_eq!(embedding, embedding2, "Mock embedder should be deterministic");
    
    // Test different content produces different embeddings
    let embedding3 = embedder.generate_embedding("Different test content").await?;
    assert_ne!(embedding, embedding3, "Different content should produce different embeddings");
...
async fn test_configuration_from_environment() -> Result<()> {
    ...
    assert!(openai_config.validate().is_ok(), "OpenAI config should be valid");
    
    // Test Ollama configuration (default)
    let ollama_config = Config::default();
    assert!(ollama_config.validate().is_ok(), "Ollama config should be valid");
    ...
    assert!(mock_config.validate().is_ok(), "Mock config should be valid");
    
    // Test invalid configuration
    let mut invalid_config = Config::default();
    invalid_config.embedding.provider = "invalid_provider".to_string();
    
    assert!(invalid_config.validate().is_err(), "Invalid provider should fail validation");
...
async fn test_embedding_content_types() -> Result<()> {
    ...
    for content in &test_contents {
        ...
        assert_eq!(embedding.len(), 768, "All embeddings should have consistent dimensions");
        
        // Verify embedding is normalized (approximately unit length)
        let magnitude: f32 = embedding.iter().map(|x| x * x).sum::<f32>().sqrt();
        assert!((magnitude - 1.0).abs() < 0.01, "Embedding should be approximately normalized");
    }
    
    println!("✅ Embedding content types work correctly");
    println!("  - Tested {} different content types", test_contents.len());
    println!("  - All embeddings have consistent {} dimensions", embedder.embedding_dimension());
...
async fn test_embedding_performance() -> Result<()> {
    ...
    let _embedding = embedder.generate_embedding("Performance test content").await?;
    let single_duration = start.elapsed();
    
    // Should be very fast for mock embeddings
    assert!(single_duration < std::time::Duration::from_millis(100), 
    ...
    assert!(batch_duration < std::time::Duration::from_secs(2), 
...
async fn test_configuration_to_embedder_integration() -> Result<()> {
    ...
    let embedding = embedder.generate_embedding("Integration test content").await?;
    assert!(!embedding.is_empty());
...
async fn test_comprehensive_basic_functionality() -> Result<()> {
    ...
    assert!(duration < std::time::Duration::from_millis(100), "Mock embedding should be fast");
