use codex_memory::memory::{
    event_triggers::{
        EventTriggeredScoringEngine, TriggerConfig, TriggerEvent, TriggerPattern
    },
...
async fn test_trigger_pattern_creation() {
    let pattern = TriggerPattern::new(
        ...
        vec!["error".to_string(), "exception".to_string(), "failure".to_string()],
    ).unwrap();
...
async fn test_confidence_calculation() {
    let mut pattern = TriggerPattern::new(
        ...
        vec!["security".to_string(), "vulnerability".to_string(), "attack".to_string()],
    ).unwrap();
...
async fn test_event_triggered_scoring_engine_security() {
    ...
    let test_cases = vec![
        ("Security vulnerability detected in authentication system", TriggerEvent::Security),
        ("XSS attack vector found in user input validation", TriggerEvent::Security),
        ("SQL injection vulnerability in database layer", TriggerEvent::Security),
    ...
    for (content, expected_trigger) in test_cases {
        ...
        println!("Triggered: {}, Confidence: {}, Type: {:?}", result.triggered, result.confidence, result.trigger_type);
        
        assert!(result.triggered, "Failed to trigger for: {} (confidence: {})", content, result.confidence);
...
async fn test_event_triggered_scoring_engine_error() {
    ...
    let test_cases = vec![
        ...
        ("System crash during user authentication", TriggerEvent::Error),
...
async fn test_event_triggered_scoring_engine_performance() {
    ...
    let test_cases = vec![
        ("Database query performance degraded significantly", TriggerEvent::Performance),
        ("Memory leak causing application slowdown", TriggerEvent::Performance),
        ("API response time exceeding SLA thresholds", TriggerEvent::Performance),
        ("Load balancer showing high latency", TriggerEvent::Performance),
    ];
...
async fn test_event_triggered_scoring_engine_business_critical() {
    ...
    let test_cases = vec![
        ("Revenue impact from customer churn increased", TriggerEvent::BusinessCritical),
        ("Strategic decision needed for product roadmap", TriggerEvent::BusinessCritical),
        ("Critical customer escalation requiring attention", TriggerEvent::BusinessCritical),
        ("KPI metrics showing declining conversion rates", TriggerEvent::BusinessCritical),
    ];
...
async fn test_event_triggered_scoring_engine_user_experience() {
    ...
    let test_cases = vec![
        ("User feedback indicates confusing navigation", TriggerEvent::UserExperience),
        ("Accessibility issues reported in mobile app", TriggerEvent::UserExperience),
        ("Customer complaints about checkout process", TriggerEvent::UserExperience),
        ("UI/UX improvements needed for user satisfaction", TriggerEvent::UserExperience),
    ];
...
async fn test_metrics_tracking() {
    ...
    assert!(metrics.triggers_by_type.contains_key(&TriggerEvent::Performance));
...
async fn test_user_specific_customization() {
    ...
    engine.add_user_customization("user123".to_string(), user_patterns).await.unwrap();
    ...
    let result2 = engine.analyze_content(
        "Custom security implementation needed",
        0.5,
        None,
    ).await.unwrap();
...
async fn test_accuracy_requirements() {
    ...
    let test_cases = vec![
        ...
        ("SQL injection vulnerability found", true, TriggerEvent::Security),
        ("XSS attack vector detected", true, TriggerEvent::Security),
        ("Authentication bypass discovered", true, TriggerEvent::Security),
