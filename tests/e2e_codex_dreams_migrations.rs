...
async fn test_migration_creates_all_schema_elements() -> Result<()> {
    ...
    // Run the migration
    sqlx::migrate!("./migration/migrations").run(&pool).await?;
    ...
    assert!(
        ...
        "processing_metadata column should exist in memories table"
    );
...
async fn test_migration_creates_optimized_indexes() -> Result<()> {
    ...
    assert!(
        hnsw_index_exists,
        "HNSW index should exist on insight_vectors"
    );
    ...
    assert!(
        ...
        "Composite index on type/confidence should exist"
    );
...
async fn test_migration_rollback() -> Result<()> {
    let database_url = env::var("TEST_DATABASE_URL").unwrap_or_else(|_| {
        "postgresql://postgres:postgres@localhost:5432/codex_test_rollback".to_string()
    });
    ...
    assert!(
        insights_exists_before,
        "insights table should exist before rollback"
    );
    ...
    assert!(
        !insights_exists_after,
        "insights table should not exist after rollback"
    );
    ...
    assert!(
        !metadata_column_exists,
        "processing_metadata column should be removed"
    );
...
async fn test_migration_idempotency() -> Result<()> {
    let database_url = env::var("TEST_DATABASE_URL").unwrap_or_else(|_| {
        "postgresql://postgres:postgres@localhost:5432/codex_test_idempotent".to_string()
    });
    ...
    let index_count: i64 =
        sqlx::query_scalar("SELECT COUNT(*) FROM pg_indexes WHERE tablename = 'insights'")
            .fetch_one(&pool)
            .await?;

    // Should have expected number of indexes, not duplicates
    assert!(
        index_count > 0 && index_count < 10,
...
async fn test_migration_feature_flag_check() -> Result<()> {
    ...
    let database_url = env::var("TEST_DATABASE_URL").unwrap_or_else(|_| {
        "postgresql://postgres:postgres@localhost:5432/codex_test_flag".to_string()
    });
    ...
    let flag_value: Option<String> =
        sqlx::query_scalar("SELECT current_setting('codex.dreams_enabled', true)")
            .fetch_optional(&pool)
            .await?;
    ...
    assert_eq!(
        ...
        "Feature flag should be respected"
    );
...
async fn test_migration_constraints() -> Result<()> {
    let database_url = env::var("TEST_DATABASE_URL").unwrap_or_else(|_| {
        "postgresql://postgres:postgres@localhost:5432/codex_test_constraints".to_string()
    });
